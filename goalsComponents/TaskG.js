...
import TaskItemG from "./TaskItemG";
...
const TaskG = () => {
    ...
    const addTask = async () => {
        try {
            const docRef = await addDoc(collection(db,"GoalsLists", userId, "goals", id, "tasks"), {
                title: title,
                isChecked: false,
            });
    ...
    const getTaskList = async (title) => {
        ...
        try {
            ...
            const querySnapshot = await getDocs(query(collection(db,"GoalsLists", userId, "goals", id, "tasks")));
    ...
    const DeleteTaskList = async () => {
        try {
            const querySnapshot = await getDocs(collection(db,"GoalsLists", userId, "goals", id, "tasks"));
            for (const docSnap of querySnapshot.docs) {
                await deleteDoc(doc(db,"GoalsLists", userId, "goals", id, "tasks", docSnap.id));
            }
            getTaskList();
        } catch (error) {
